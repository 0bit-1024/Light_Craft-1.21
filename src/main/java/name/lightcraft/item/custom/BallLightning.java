...
public class BallLightning extends Item implements ProjectileItem {
    private static final int COOLDOWN = 10;

    public BallLightning(Item.Settings settings) {
        super(settings);
    }
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        if (!world.isClient()) {
            BallLightningEntity BallLightningEntity = new BallLightningEntity(user, world, user.getPos().getX(), user.getEyePos().getY(), user.getPos().getZ());
            BallLightningEntity.setVelocity(user, user.getPitch(), user.getYaw(), 0.0F, 1.5F, 1.0F);
            world.spawnEntity(BallLightningEntity);
        }
    ...
    public ProjectileEntity createEntity(World world, Position pos, ItemStack stack, Direction direction) {
        ...
        BallLightningEntity BallLightningEntity = new BallLightningEntity(world, pos.getX(), pos.getY(), pos.getZ(), vec3d);
        BallLightningEntity.setVelocity(vec3d);
