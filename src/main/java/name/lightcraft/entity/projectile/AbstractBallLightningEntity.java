...
public class AbstractBallLightningEntity extends ExplosiveProjectileEntity implements FlyingItemEntity {
    ...
    public AbstractBallLightningEntity(EntityType<? extends AbstractBallLightningEntity> entityType, World world) {
        super(entityType, world);
        this.accelerationPower = 0.0;
    }

    public AbstractBallLightningEntity(EntityType<? extends AbstractBallLightningEntity> type, World world, Entity owner, double x, double y, double z) {
        super(type, x, y, z, world);
        this.setOwner(owner);
        this.accelerationPower = 0.0;
    }

    AbstractBallLightningEntity(EntityType<? extends AbstractBallLightningEntity> entityType, double d, double e, double f, Vec3d vec3d, World world) {
    ...
    public boolean collidesWith(Entity other) {
        return !(other instanceof AbstractBallLightningEntity) && super.collidesWith(other);
    }

    @Override
    protected boolean canHit(Entity entity) {
        if (entity instanceof AbstractBallLightningEntity) {
            return false;
        } else {
    ...
    protected void onEntityHit(EntityHitResult entityHitResult) {
        ...
        if (!this.getWorld().isClient) {
            ...
            DamageSource damageSource = this.getDamageSources().lightningBolt();
